...
public class SplineSegment
{
    ...
    public void Recalculate(bool force = false)
    {
        ...
        while (t < 1)
        {
            ...
            Points.Add(new SplinePoint(point, normal, t, accumLength));
            lastNormal = normal;
        ...
        Points.Add(new SplinePoint(SecondControlPoint.Position, SecondControlPoint.Control, 1, accumLength));
        Length = accumLength;
    ...
    public void DrawGizmos(Color lineColor)
    {
        ...
        var lastPoint = default(Vector3);
        for (int i = 0; i < Points.Count; i++)
        {
            var p = Points[i];
            if (i > 0)
            {
                Gizmos.DrawLine(lastPoint, p.Position);
            }
    ...
    public class ControlPoint
    {
        ...
        public ControlPoint(Vector3 position, Vector3 control, Vector3 upVector)
        {
            Position = position;
            Control = control;
            UpVector = upVector;
        ...
        {
            return new ControlPoint(Position, Control, UpVector);
        }
